...
var ProjectFileTree = class _ProjectFileTree {
  ...
  async saveToFile(filePath) {
    ...
    const tmpPath = `${filePath}.${(0, import_crypto.randomBytes)(6).toString("hex")}.tmp`;
  ...
  static async loadFromFile(filePath) {
    try {
      ...
      return _ProjectFileTree.fromJSON(json);
    } catch {
      return null;
    }
  }
  scheduleSave(filePath, debounceMs = 2500) {
    if (this.saveTimer) {
      clearTimeout(this.saveTimer);
    }
    if (!this.savePromise) {
      this.savePromise = new Promise((resolve2, reject) => {
