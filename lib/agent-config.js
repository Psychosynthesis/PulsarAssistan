...
__export(agent_config_exports, {
  ...
  isLaunchedAgentStale: () => isLaunchedAgentStale,
  launchTargetsEqual: () => launchTargetsEqual,
  migrateAgentsConfig: () => migrateAgentsConfig,
  normalizeAgentsConfig: () => normalizeAgentsConfig,
  patchCommandPath: () => patchCommandPath,
  quoteCommandLine: () => quoteCommandLine,
  resolveActiveAgent: () => resolveActiveAgent,
  resolveAgent: () => resolveAgent,
  resolveApiKey: () => resolveApiKey,
  toLaunchTarget: () => toLaunchTarget
});
...
var DEFAULT_PROCESS_OUTPUT_BYTES = 64 * 1024;
var MAX_PROCESS_OUTPUT_BYTES = 1024 * 1024;
...
// src/agent-config.ts
var DEFAULT_AGENT_COMMAND = "copilot --acp --stdio";
var COPILOT_AGENT_ID = "copilot";
var COPILOT_AGENT_NAME = "GitHub Copilot";
...
function deepEqual(a, b) {
  ...
  if (isObject(a) && isObject(b)) {
    ...
    return keys.every(
      (key) => key in b && deepEqual(a[key], b[key])
    );
  }
  return false;
}
...
function seedAgentFromCommand(command) {
...
function normalizeAgentsConfig(raw) {
  ...
  const config = { ...source, agents };
  delete config.version;
  if (typeof source.activeAgentId === "string" && source.activeAgentId.trim() !== "") {
...
function migrateAgentsConfig(raw, legacyCommand) {
  const source = isObject(raw) ? raw : {};
  const hasAgents = Object.prototype.hasOwnProperty.call(source, "agents") && isObject(source.agents);
  const normalized = normalizeAgentsConfig(raw);
  if (hasAgents) {
    const ids = Object.keys(normalized.agents);
    if (ids.length > 0 && (!normalized.activeAgentId || !normalized.agents[normalized.activeAgentId])) {
  ...
  const command = typeof legacyCommand === "string" && legacyCommand.trim() !== "" ? legacyCommand.trim() : DEFAULT_AGENT_COMMAND;
...
function resolveAgent(config, preferredId) {
...
function resolveActiveAgent(config) {
  return resolveAgent(config);
}
function resolveApiKey(agent, env = process.env) {
...
function toLaunchTarget(id, agent, env = process.env, model) {
  const type = agent.type ?? (agent.command ? "acp" : "openai");
  if (type === "openai") {
    const baseUrl = optionalString(agent.baseUrl)?.replace(/\/+$/, "");
    const configuredModel = optionalString(agent.defaultModel) ?? optionalString(agent.model);
...
function launchTargetsEqual(a, b) {
  if (!a || !b) return false;
  ...
  if (a.kind === "openai" && b.kind === "openai") {
    return a.baseUrl === b.baseUrl && a.model === b.model && a.apiKey === b.apiKey && a.modelsUrl === b.modelsUrl && a.stream === b.stream;
  }
  return false;
}
function isLaunchedAgentStale(config, launchedSnapshotId) {
  if (!launchedSnapshotId) return false;
  return !config.agents[launchedSnapshotId];
}
function quoteCommandLine(args) {
  return args.map((arg) => {
...
function patchCommandPath(command, oldPath, newPath) {
